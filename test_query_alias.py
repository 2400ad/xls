...
def test_alias_handling():
    ...
    query1 = "SELECT to_char(tdate, 'YYYYMMDDHH24MISS') AS tdate FROM dual"
    query2 = "SELECT to_char(tdate, 'YYYYMMDDHH24MISS') FROM dual"
    ...
    query1 = "SELECT to_char(tdate, 'YYYYMMDDHH24MISS') AS tdate FROM dual"
    query2 = "SELECT to_char(tdate, 'YYYYMMDDHH24MISS') tdate FROM dual"
    result = parser.compare_queries(query1, query2)
    ...
    # 테스트 3: 여러 공백이 있는 경우
    query1 = "SELECT to_char(tdate, 'YYYYMMDDHH24MISS')    tdate FROM dual"
    query2 = "SELECT to_char(tdate, 'YYYYMMDDHH24MISS') tdate FROM dual"
    result = parser.compare_queries(query1, query2)
    ...
    # 테스트 4: 대소문자 차이
    query1 = "SELECT TO_CHAR(tdate, 'YYYYMMDDHH24MISS') AS tdate FROM dual"
    query2 = "SELECT to_char(tdate, 'YYYYMMDDHH24MISS') AS tdate FROM dual"
    result = parser.compare_queries(query1, query2)
    ...
    # 테스트 6: 실제 다른 쿼리
    query1 = "SELECT to_char(tdate, 'YYYYMMDDHH24MISS') AS tdate FROM dual"
    query2 = "SELECT to_char(updated_at, 'YYYYMMDDHH24MISS') AS tdate FROM dual"
    result = parser.compare_queries(query1, query2)
